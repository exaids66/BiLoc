...
def test(cfg: DictConfig):
    ...
    if cfg.get("preextract_z", False):
        z_dir = osp.join(cfg.exp_dir, "teacher_z")
        os.makedirs(z_dir, exist_ok=True)
        for step, batch in enumerate(eval_dataloader):
            images = batch["image"].to(device)
            shapelist = list(images.shape)
            reshaped_image = images.reshape(shapelist[0] * shapelist[1], *shapelist[2:])
            with torch.no_grad():
                z = model.image_feature_extractor(reshaped_image)
            z = z.reshape(shapelist[0], shapelist[1], -1).cpu()

            start_idx = step * cfg.train.val_batch_size
            end_idx = min((step + 1) * cfg.train.val_batch_size, len(eval_dataset))
            for offset, sample_idx in enumerate(range(start_idx, end_idx)):
                torch.save(z[offset], osp.join(z_dir, f"{sample_idx:08d}.pt"))
            log_string(f"Saved z features: {start_idx} - {end_idx - 1}")
        return

...
# Locate BHViT repo (sibling directory of this project)
BHViT_ROOT = Path(__file__).resolve().parents[2] / "BHViT"
_BHViT_IMPORT_ERROR = None
if BHViT_ROOT.is_dir():
    for p in (BHViT_ROOT, BHViT_ROOT / "transformer"):
        if str(p) not in sys.path:
            sys.path.insert(0, str(p))
try:
    from BHViT import BHViTForImageClassification  # type: ignore
except Exception as exc1:  # pragma: no cover - informative import error
    try:
        from transformer.BHViT import BHViTForImageClassification  # type: ignore
    except Exception as exc2:
        BHViTForImageClassification = None
        _BHViT_IMPORT_ERROR = f"{repr(exc1)}; {repr(exc2)}"
...
class BHViTRangeEncoder(nn.Module):
    """
    Wrap BHViTForImageClassification to provide the token/skip interface expected by RangeViT.
    """
    def __init__(
        self,
        config: ViTConfig,
        patch_size_override=None,
        patch_stride_override=None,
    ) -> None:
        super().__init__()
        if BHViTForImageClassification is None:
            detail = f" Import error: {_BHViT_IMPORT_ERROR}" if _BHViT_IMPORT_ERROR else ""
            raise ImportError(f"BHViT is not available.{detail}")
        self.model = BHViTForImageClassification(config)
        patch_size = patch_size_override if patch_size_override is not None else getattr(config, "patch_size", (4, 16))
        self.patch_size = tuple(patch_size) if isinstance(patch_size, (list, tuple)) else (patch_size, patch_size)
        patch_stride = (
            patch_stride_override
            if patch_stride_override is not None
            else getattr(config, "patch_stride", self.patch_size)
        )
        self.patch_stride = tuple(patch_stride) if isinstance(patch_stride, (list, tuple)) else (patch_stride, patch_stride)
        self.output_dim = 384
    ...
    def forward(self, im, return_features=False):
        outputs = self.model.vit(pixel_values=im, output_hidden_states=False, return_dict=False)
        tokens = outputs[0]
        skip = None
        return tokens, skip
...
class RangeViT(nn.Module):
    ...
    def forward(self, im):
        ...
        num_extra_tokens = getattr(self.encoder, "num_extra_tokens", 1)  # 一般是 1 个 CLS token
        # x的尺寸：54,256,384
...
class ImageFeatureExtractor(nn.Module):
    def __init__(
        ...
        if backbone == 'bhvit':
            if BHViTForImageClassification is None:
                detail = f" Import error: {_BHViT_IMPORT_ERROR}" if _BHViT_IMPORT_ERROR else ""
                raise ImportError(f"BHViT backbone requested but BHViT is unavailable at {BHViT_ROOT}.{detail}")
            cfg_path = BHViT_ROOT / "configs" / "BHViT" / "config.json"
            if not cfg_path.is_file():
                raise FileNotFoundError(f"BHViT config not found at {cfg_path}")
            bhvit_cfg = ViTConfig.from_pretrained(str(cfg_path))
            bhvit_cfg.image_size = tuple(image_size)
            bhvit_cfg.num_channels = in_channels
            bhvit_cfg.patch_size = [4, 16]
            bhvit_cfg.patch_stride = [4, 16]
            bhvit_cfg.shift3 = True
            bhvit_cfg.shift5 = True
            bhvit_cfg.output_dim = 384
            if isinstance(bhvit_cfg.norm_layer, str):
                bhvit_cfg.norm_layer = nn.LayerNorm
            patch_size_override = tuple(new_patch_size) if new_patch_size is not None else None
            patch_stride_override = tuple(new_patch_stride) if new_patch_stride is not None else None
            encoder = BHViTRangeEncoder(
                bhvit_cfg,
                patch_size_override=patch_size_override,
                patch_stride_override=patch_stride_override,
            )
            patch_size_cfg = tuple(encoder.patch_size)
            patch_stride_cfg = tuple(encoder.patch_stride)
            decoder_cfg = {
                'name': decoder,
                'patch_size': patch_size_cfg,
                'patch_stride': patch_stride_cfg,
                'd_encoder': encoder.output_dim,
                'n_cls': n_cls,
            }
            decoder, name = create_decoder(decoder_cfg)
            self.rangevit = RangeViT(encoder, decoder, n_cls=n_cls)
            self._output_dim = encoder.output_dim
            return
